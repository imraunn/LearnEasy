...
function ViewSubmissions() {
  ...
  const { assignmentId, file, fileName, title, topic, deadline } =
    location.state;
  ...
  return (
    <div>
      ...
      {studentList.map((submittedFile) => {
        return (
          <SingleStudentFile
            key={submittedFile.assignment_submissionId}
            {...submittedFile}
          />
        );
      })}
...
const SingleStudentFile = ({ file, fileName, comment, late, name, photo }) => {
  if (!photo) {
    photo = logo;
  }
  return (
